				...
				async.waterfall([
					...
					function(id, fn){
						...
						async.series([
							...
							function(fn){
								db.LPUSH('order:type:' + order.type, order.id, fn);
							}
						], 
						function(err, len){
...
var createLinedOrderGroups = function(dishesCollection, oidCollection){
	var groups = [];
	...
	return [
		...
		function(len, fn){
			...
			for(var i = 0;i < pack_size;i++){
				fns.push(function(fn){db.RPOP('order:type:'+ type, fn);});
			}
	...
	async.waterfall(
		...
		[
			...
			function(result, fn){
				...
				// create order groups
				var groups = createLinedOrderGroups(dishesCollection, oidCollection);
	...
	db.SELECT(2, function(){
		db.HGETALL('order:t' + type + ':working:' + id, function(err, group){
			...
			Dish.getEntity(group.dish, function(err, dish){
				...
				callback(null, {gid: id, count: group.count, name: dish.name, ename: dish.ename, image: dish.image});
			});
		});
	});
}
...
Order.getWorkingGroups = function(type, callback){
	...
	db.SELECT(2, function(){
		async.waterfall([
			...
			function(ids, fn){
				// filter valid id
				Order.getsGroup(type, ids.reverse().filter(function(id){
					return id != null;
				}), fn);
			}
		], callback);
